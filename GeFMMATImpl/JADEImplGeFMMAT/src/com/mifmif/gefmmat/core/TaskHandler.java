...
/**
 * Class that will catch the call for proposals and handle communication with the initiator agent.
 * 
 * @author y.mifrah
 *
 */
 ...
 */
	...
	public void action() {
		...
		getOwnerAgent().addBehaviour(new ContractNetResponder(getOwnerAgent(), template) {
			...
			protected ACLMessage handleCfp(ACLMessage cfp) throws NotUnderstoodException, RefuseException {
				// System.out.println("Agent " + getOwnerAgent().getLocalName()
				...
				try {
					...
					if (handleProposal && proposal != null) {
						...
						// getOwnerAgent().getLocalName() + ": Proposing " +
						// proposal);
			...
			protected ACLMessage prepareResultNotification(ACLMessage cfp, ACLMessage propose, ACLMessage accept) throws FailureException {
				// System.out.println("Agent " + getOwnerAgent().getLocalName() + ": Proposal accepted");

				Result result = performTask(task);
				if (result != null) {
					// System.out.println("Agent " + getOwnerAgent().getLocalName() + ": Action successfully performed");
					...
					// System.out.println("Agent " + getOwnerAgent().getLocalName() + ": Action execution failed");
					throw new FailureException("unexpected-error");
				}
			}

			protected void handleRejectProposal(ACLMessage reject) {
				// System.out.println("Agent " + getOwnerAgent().getLocalName() + ": Proposal rejected");
			}
		});
		// block();
	}
	...
	public void setProbabilityToHandleProposal(double probabilityToHandleProposal) {
